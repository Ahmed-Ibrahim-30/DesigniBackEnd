...
void LandDivisionBasedOnMinimizeAcuteAngles::divideLand(double ratioA, double ratioB, int maxDivisions,vector<Polygon1> &pols, vector<vector<Polygon1>> &ans)
{
    ...
    for (int i = 0; i < pols.size(); ++i)
    {
        ...
        for (int j = 0; j < paiPoly.size(); ++j) {
            ...
            double acuteLinesCount = 0;
            for(auto &p : pps)
            {
                acuteLinesCount+= p.countAcuteAnglesLines();
            }

            sortSolutions.emplace_back(acuteLinesCount , i , j);
...
vector<vector<Polygon1>>LandDivisionBasedOnMinimizeAcuteAngles::divideLand(const Polygon1 &land, double ratioA, double ratioB,int maxDivisions)
{
...
vector<vector<Polygon1>> LandDivisionBasedOnMinimizeAcuteAngles::divideLands(vector<Polygon1> &lands, double ratioA, double ratioB,int maxDivisions)
{
    vector<vector<Polygon1>> ans;
