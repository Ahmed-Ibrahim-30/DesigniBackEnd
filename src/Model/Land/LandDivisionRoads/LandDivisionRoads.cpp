...
vector<pair<Polygon1 , Polygon1>> LandDivisionRoads::dividePolygons(Polygon1 polygon1)
{
    ...
    for(auto &solution : ans2)
    {
        ...
        for(auto &p : points1)
        {
            if(p == p1 || p == p2)
            {
                p .setX(center1.getX() + (p.getX() - center1.getX()) * 0.4);
                p .setY(center1.getY() + (p.getY() - center1.getY()) * 0.4);
            }
        }
        ...
        for(auto &p : points2)
        {
            if(p == p1 || p == p2)
            {
                p .setX(center2.getX() + (p.getX() - center2.getX()) * 0.4);
                p .setY(center2.getY() + (p.getY() - center2.getY()) * 0.4);
            }
        }
