...
vector<Room> BuildingDesignerCorridorBesideLiving::generateCorridorLayout(vector<RoomEntity> &roomE, Room &mainRoom)
{
    ...
    for (int i = 1; i < n; ++i) {
        ...
        {
            topRoomsSize = i;
            if (bottomPrefix > topPrefix)isTopGreater = false;
            else isTopGreater = true;
            width = curW;
        }
    }
...
//    }
    ...
    for (int i = 0; i < topRoomsSize; ++i)
    {
        ...
        pair<double , vector<double>> res = findClosestSum(width - (isTopGreater ?0.5 : 0), tempV , roomE);
        if (i == topRoomsSize-1)
        {
            index = i + 1;
...
vector<Design> BuildingDesignerCorridorBesideLiving::generateDesign()
{
    ...
    else
    {
        ...
        livingX = fetchSuitableLivingWidth(livingRooms) + 1;
